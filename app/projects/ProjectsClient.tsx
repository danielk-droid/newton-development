...
function getStatusDescription(status: string) {
  switch (status) {
    ...
    case "Approved":
    case "Approved with Conditions": return "The project has received approval.";
    case "Submitted": return "An application has been filed.";
    case "Under Review": return "The project is being reviewed.";
    case "Scheduled for Hearing": return "A public hearing is scheduled.";
    case "Completed": return "The project is complete.";
    case "Appealed": return "The project is under appeal.";
    case "Denied": return "The project has been denied.";
    case "Proposed": return "The project has been proposed.";
    default: return null;
  }
}
...
export default function ProjectsClient({ projects }: ProjectsClientProps) {
  ...
  const villages = useMemo(() => Array.from(new Set(projects.map((project) => project.village).filter((value): value is string => Boolean(value) && value !== "Unknown"))).sort(), [projects]);
  const types = useMemo(() => {
    const availableTypes = new Set(projects.map((project) => project.type).filter(Boolean));
    return ["Housing", "Mixed-Use", "Commercial", "Public Building", "Zoning", "Transportation", "Historic Preservation", "Other"].filter((item) => availableTypes.has(item));
  }, [projects]);
  ...
  function clearFilters() { setSearch(""); setStatus("All"); setVillage("All"); setType("All"); }
