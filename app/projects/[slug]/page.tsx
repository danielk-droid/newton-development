  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <div className="mx-auto max-w-6xl px-5 py-10 sm:px-6 md:py-14">
        ...
        {isPrivateDevelopment && highlights.length === 0 && <section className="mt-10"><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Development facts</p><h2 className="mt-2 text-3xl font-bold tracking-tight">At a glance</h2><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Residential units", project.facts.units], ["Affordable units", project.facts.affordableUnits], ["Stories", project.facts.stories], ["Parking spaces", project.facts.parkingSpaces]].filter(([, value]) => value !== null).map(([label, value]) => <div key={label as string} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-sm font-medium text-slate-500">{label as string}</p><p className="mt-2 text-3xl font-bold">{(value as number).toLocaleString()}</p></div>)}</div></section>}
        ...
        <section className="mt-10"><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Project progress</p><h2 className="mt-2 text-3xl font-bold tracking-tight">{isPublicProject ? "Project lifecycle" : "Development process"}</h2><div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="relative grid gap-5 sm:grid-cols-5">{lifecycleStages.map((stage, index) => { const isCompleted = currentStageIndex >= 0 && index < currentStageIndex; const isCurrent = currentStageIndex === index; const isFuture = !isCompleted && !isCurrent; return <div key={stage} className="relative"><div className="flex items-center gap-3 sm:block"><div className={`relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 text-sm font-bold ${isCurrent ? "border-slate-900 bg-slate-900 text-white ring-4 ring-slate-200" : isCompleted ? "border-slate-900 bg-slate-900 text-white" : "border-slate-300 bg-white text-slate-400"}`}>{isCompleted ? "✓" : index + 1}</div><div className={`mt-0 text-sm font-semibold sm:mt-3 ${isCurrent ? "text-slate-950" : isCompleted ? "text-slate-700" : "text-slate-400"}`}>{stage}{isCurrent && <span className="ml-2 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-700">Current</span>}</div></div>{index < lifecycleStages.length - 1 && <div className={`absolute left-10 top-5 hidden h-0.5 w-[calc(100%-1.25rem)] sm:block ${index < currentStageIndex ? "bg-slate-900" : "bg-slate-200"}`} aria-hidden="true" />}{isFuture && <p className="mt-1 text-xs text-slate-400 sm:mt-2">Not reached</p>}</div>; })}</div></div></section>
