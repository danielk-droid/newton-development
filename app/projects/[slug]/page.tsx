    ...
    <main className="min-h-screen bg-white text-slate-900">
      <div className="mx-auto max-w-5xl px-6 py-12">
          ...
          <div className="rounded-2xl border border-slate-200 p-6">
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-slate-600">{getStatusDescription(project.status)}</p>
              </div>
        ...
        <section className="mt-10">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-slate-500">Next opportunity</p>
            <h2 className="mt-2 text-3xl font-bold tracking-tight">Participate in the process</h2>
            <p className="mt-3 max-w-2xl leading-7 text-slate-600">We only show an upcoming opportunity when an official City record connects it to this project.</p>
          </div>

          {nextOpportunity ? (
            <div className="mt-8 rounded-2xl border border-slate-300 bg-slate-50 p-6">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-slate-900 px-3 py-1 text-sm font-semibold text-white">{getEventTypeLabel(nextOpportunity.type)}</span>
                <span className="text-sm font-medium text-slate-500">{formatDate(nextOpportunity.date)}</span>
              </div>
            ...
            <div className="mt-8 rounded-2xl border border-slate-200 p-6">
              <h3 className="text-xl font-bold">No verified upcoming opportunity</h3>
              <p className="mt-2 max-w-2xl leading-7 text-slate-600">We have not identified an upcoming public meeting, hearing, or other participation opportunity that an official record currently connects to this project.</p>
              <a href="https://www.newtonma.gov/government/city-clerk/city-council/calendar-news/calendar" target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex items-center text-sm font-semibold underline underline-offset-4">Check the City calendar <span aria-hidden="true" className="ml-2">↗</span></a>
            </div>
          )}
        </section>
              ...
              {mainStages.map((stage, index) => {
                ...
                return (
                  <div key={stage} className="flex items-start gap-4">
                    <div className={`mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-sm font-bold ${isCompleted || isCurrent ? "bg-slate-900 text-white" : "border border-slate-300 text-slate-400"}`}>
                      {isCompleted ? "✓" : index + 1}
                    </div>
                    <p className={`font-semibold ${isCurrent ? "text-slate-900" : isCompleted ? "text-slate-700" : "text-slate-400"}`}>
                      {stage}{isCurrent && <span className="ml-2 text-sm font-medium text-slate-500">Current stage</span>}
                    </p>
                  </div>
                );
              })}
        ...
        <section className="mt-10">
          <p className="text-sm font-semibold uppercase tracking-wide text-slate-500">Verified records</p>
          <h2 className="mt-2 text-3xl font-bold tracking-tight">What has happened</h2>
                ...
                {[...pastEvents].reverse().map((event) => (
                  <div key={event.id} className="p-6">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">{getEventTypeLabel(event.type)}</span>
                        <h3 className="mt-2 text-xl font-bold">{event.title}</h3>
                      </div>
                      <p className="shrink-0 text-sm font-medium text-slate-500">{formatDate(event.date)}</p>
                    </div>
                    <p className="mt-3 max-w-3xl leading-7 text-slate-600">{event.description}</p>
                    <a href={event.sourceUrl} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex text-sm font-semibold underline underline-offset-4">View official record <span aria-hidden="true" className="ml-2">↗</span></a>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="mt-10">
          ...
          <h2 className="mt-2 text-3xl font-bold tracking-tight">Verify the information</h2>
          ...
          <div className="mt-8 space-y-3">
            ...
            {project.links.map((link) => (
              <a key={`${link.label}-${link.url}`} href={link.url} target="_blank" rel="noopener noreferrer" className="block rounded-xl border border-slate-200 p-5 hover:bg-slate-50">
                <p className="font-semibold">{link.label}</p>
                <p className="mt-1 break-all text-sm text-slate-600">{link.url}</p>
              </a>
            ))}
          </div>
        </section>
        ...
        <section className="mt-10 border-t border-slate-200 pt-8">
          <p className="text-sm font-semibold text-slate-900">Data provenance</p>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Current project information is collected from official City sources. Historical events are added separately and are displayed only when they are tied to a verified official record.</p>
          <p className="mt-2 text-sm text-slate-500">Current collection: {formatDateTime(project.lastUpdated)}</p>
        </section>
      </div>
    </main>
  );
