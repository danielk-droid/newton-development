...
function parseProjects(source) {
  ...
  for (const match of source.matchAll(/\{\s*id:\s*"([^"]+)"[\s\S]*?\n\s*\},/g)) {
    const block = match[0]; const id = match[1]; const name = block.match(/\n\s*name:\s*"([^"]+)"/)?.[1]; const address = block.match(/\n\s*address:\s*"([^"]+)"/)?.[1];
...
function parseAddress(address) {
  ...
  const match = value.match(/\b(\d{1,5}(?:-\d{1,5})?)\s+([A-Za-z][A-Za-z' -]*?\b(?:Street|St|Road|Rd|Avenue|Ave|Drive|Dr|Parkway|Pkwy|Place|Pl|Way|Lane|Ln|Court|Ct|Circle|Cir|Terrace|Ter|Boulevard|Blvd)\b)/i);
...
async function findAddress(project) {
  const parsed = parseAddress(project.address); if (!parsed) return null;
  ...
  const data = await query(ADDRESS_LAYER, { where: `Number=${Number(parsed.number)} AND UPPER(FullStName) LIKE UPPER('%${street.replace(/'/g, "''")}%') AND Status <> 'Inactive'`, outFields: "Number,NumberSuffix,FullStName,Address,Status,LocationType", returnGeometry: "true", outSR: "4326", resultRecordCount: "20" });
  const feature = (data.features ?? []).find((item) => item.geometry?.x != null && item.geometry?.y != null);
  if (!feature) return null;
  return { lat: Number(feature.geometry.y), lon: Number(feature.geometry.x), matchedAddress: feature.attributes?.Address ?? project.address, method: "official-address-point" };
}
...
async function findStreet(project) {
  ...
  const data = await query(STREET_LAYER, { where: `UPPER(NAME) = UPPER('${street.replace(/'/g, "''")}')`, outFields: "NAME,OBJECTID", returnGeometry: "true", outSR: "4326", resultRecordCount: "200" });
  const points = (data.features ?? []).flatMap((feature) => (feature.geometry?.paths ?? []).flat());
...
for (const project of projects) {
  if (/^citywide$/i.test(project.address.trim())) { unresolved.push({ id: project.id, address: project.address, reason: "citywide" }); continue; }
  try {
    const point = await findAddress(project) ?? await findStreet(project);
    if (point && Number.isFinite(point.lat) && Number.isFinite(point.lon)) resolved.push({ id: project.id, ...point });
    else unresolved.push({ id: project.id, address: project.address, reason: "no official GIS match" });
  } catch (error) { unresolved.push({ id: project.id, address: project.address, reason: error.message }); }
}
...
await fs.writeFile(OUTPUT_PATH, `${JSON.stringify({ checkedAt: new Date().toISOString(), source: `${GIS_BASE}/12 and ${GIS_BASE}/15`, sourceDescription: "City of Newton GIS address points and street centerlines", projects: resolved, unresolved }, null, 2)}\n`, "utf8");
