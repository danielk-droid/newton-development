...
const allowedCoordinateHosts = new Set(["gisweb.newtonma.gov"]);

if (!Array.isArray(data.projects) || data.projects.length === 0) throw new Error("Private development project data is empty.");
function extractProjectIds(source) { return [...source.matchAll(/\bid:\s*"([^"]+)"/g)].map((match) => match[1]); }
const projectIds = new Set([...data.projects.map((project) => project.id), ...extractProjectIds(publicProjectsSource), ...extractProjectIds(transportationSource)]);
...
for (const source of [publicProjectsSource, transportationSource]) for (const match of source.matchAll(/\b(?:sourceUrl|url):\s*"(https:\/\/[^"\n]+)"/g)) urlFields.push(["catalog source URL", match[1]]);
for (const match of highlightsSource.matchAll(/\{\s*label:\s*"([^"]+)"\s*,\s*value:\s*"([^"]+)"\s*,\s*sourceUrl:\s*"(https:\/\/[^"\n]+)"\s*\}/g)) urlFields.push([`project highlight ${match[1]}`, match[3]]);
for (const [label, value] of urlFields) {
  ...
  if (!allowedCatalogHosts.has(parsed.hostname.toLowerCase())) throw new Error(`${label} points outside approved authoritative hosts: ${value}`);
}

const highlightProjectIds = [...highlightsSource.matchAll(/^\s*"([^"]+)":\s*\[/gm)].map((match) => match[1]);
for (const projectId of highlightProjectIds) if (!projectIds.has(projectId)) throw new Error(`Project highlights reference unknown project ${projectId}.`);
...
for (const block of eventBlocks) {
  ...
  const projectId = read("projectId"); const date = read("date"); const sourceUrl = read("sourceUrl"); const participationUrl = read("participationUrl"); const matchedAddress = read("matchedAddress"); const sourceCheckedAt = read("sourceCheckedAt"); const id = read("id"); const type = read("type");
  ...
  if (Number.isNaN(parsedDate.getTime()) || parsedDate.toISOString().slice(0, 10) !== date) throw new Error(`Event ${id} has an invalid calendar date: ${date}.`);
  let parsedUrl;
  try { parsedUrl = new URL(sourceUrl); } catch { throw new Error(`Event ${id} has an invalid source URL.`); }
  if (parsedUrl.protocol !== "https:" || !allowedEventHosts.has(parsedUrl.hostname.toLowerCase())) throw new Error(`Event ${id} points outside approved City source hosts: ${sourceUrl}`);
  if (participationUrl) {
    let participation;
    try { participation = new URL(participationUrl); } catch { throw new Error(`Event ${id} has an invalid participation URL.`); }
    if (participation.protocol !== "https:" || !allowedEventHosts.has(participation.hostname.toLowerCase())) throw new Error(`Event ${id} participation URL points outside approved City source hosts: ${participationUrl}`);
  }
  if (sourceCheckedAt) {
    if (Number.isNaN(new Date(sourceCheckedAt).getTime())) throw new Error(`Event ${id} has an invalid sourceCheckedAt timestamp.`);
...
if (!Array.isArray(status.sources) || status.sources.length !== 6) throw new Error(`Expected 6 configured official event sources, found ${status.sources?.length ?? 0}.`);
if (!status.checkedAt || Number.isNaN(new Date(status.checkedAt).getTime())) throw new Error("Event collection status has no valid checkedAt timestamp.");
if (!Array.isArray(status.allowedHosts) || status.allowedHosts.some((host) => !allowedEventHosts.has(host))) throw new Error("Event collector allowed-host configuration is not restricted to approved City hosts.");
...
for (const point of coordinates.projects) {
  if (!projectIds.has(point.id)) throw new Error(`GIS coordinates reference unknown project ${point.id}.`);
  if (coordinateIds.has(point.id)) throw new Error(`Duplicate GIS coordinate record for ${point.id}.`);
  coordinateIds.add(point.id);
